...
import { api } from "@/lib/api"; // kept for v2.1
...
export const ShareListModal = ({ open, onOpenChange, seeds, onOrderSent }: ShareListModalProps) => {
  ...
  // Per-item quantities — number input, user types directly (orders can be 1000+)
  ...
  // ── v2.1: uncomment to re-enable direct order sending ───────────────────
  // const handleSend = async () => {
  //   if (!seeds.length) { toast.error("Your list is empty"); return; }
  //   setSending(true);
  //   try {
  ...
  const exportJSON = () => {
    const data = {
      ...
      delivery_address: address,
      notes,
      items: seeds.map(s => ({ seed_id: s.seed_id || s.id, name: s.name, quantity: getQty(s) })),
    };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        {/* v2.1: swap back to sent screen when order flow is live
        {sent ? (
          <div className="text-center py-6 space-y-3">
            <CheckCircle2 className="w-16 h-16 text-green-500 mx-auto" />
            <h3 className="text-lg font-semibold">Order Sent!</h3>
            <p className="text-muted-foreground text-sm">The collector has been notified.</p>
            <Button onClick={() => { setSent(false); onOpenChange(false); }} className="w-full">Close</Button>
          </div>
        ) : ( */}
          <div className="space-y-4">

            {/* Seeds with editable qty — number input for large quantities */}
              ...
              {seeds.map((s, i) => (
                <div key={i} className="flex items-center gap-3 text-sm">
                  ...
                  <Input
                    ...
                    className="w-20 h-8 text-center font-mono text-sm px-2"
                  />
                </div>
              ))}
            </div>
            ...
            {/* v2.1 Coming Soon — replace this block with Send button when ready */}
            <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-center space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                For now, export below and share manually.
              </p>
            </div>

            {/* v2.1: uncomment Send button when handleSend is active
